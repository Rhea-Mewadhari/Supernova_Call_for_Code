...
# cloudinary.config(
#     cloud_name=os.getenv('CLOUD_NAME'),  # Replace with your actual Cloud Name
...
# @app.route('/custom-chatbot/send', methods=['POST'])
# def send_to_custom_chatbot():
#     data = request.get_json()
#     label = data.get('label')
#     session_id = data.get('session_id')

#     # Add your logic to map labels to responses or flows
#     chatbot_response = f"Received label: '{label}'. Starting appropriate flow..."
...
#     if not email or not session_id or not messages:
...
#         # Query MongoDB for chat history
...
#     except Exception as e:
#         print(f"Error retrieving chat history: {e}")  # Log error to console
#         return jsonify({'error': 'Internal server error'}), 500

# print(os.getenv('MODAL_API'))
...
#         if not results:
#             return jsonify({"answer": "I do not know. Please seek out advice from the nearest healthcare practitioner."})
