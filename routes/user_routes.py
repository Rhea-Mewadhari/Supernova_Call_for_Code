from flask import Blueprint, session, request, jsonify, render_template, flash, current_app
...
def profile():
    ...
    if request.method == 'POST':
        ...
        if 'profile_picture' in request.files:
            ...
            new_url, error = upload_profile_picture(file, max_size, user.get('profile_picture') if user else None)
            if error:
                flash(error, 'danger')
            else:
...
def submit_feedback():
    ...
    store_feedback(session['email'], csat, feedback_text)
...
def update_profile():
    ...
    if update_fields:
        update_user_profile(email, update_fields)
